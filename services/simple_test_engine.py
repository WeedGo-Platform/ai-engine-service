"""
Simple Test Engine with Prompt Support
Barebones AI engine for testing model capabilities with or without prompts
Can load prompt templates from JSON files to test their effect on model behavior
"""
...
class SimpleTestEngine:
    ...
    def __new__(cls):
        ...
        if cls._instance is None:
            cls._instance = super(SimpleTestEngine, cls).__new__(cls)
        return cls._instance
    
    """
    Simple engine for testing raw model capabilities with optional prompt templates
    """
    ...
    def __init__(self):
        ...
        logger.info(f"SimpleTestEngine initialized with {len(self.available_models)} models")
    ...
    def generate(self, 
                 ...
                 top_p: float = 0.95,
                 top_k: int = 40) -> Dict:
        """Generate response with optional prompt template and system config"""
...
def get_test_engine() -> SimpleTestEngine:
    """Get or create test engine singleton"""
    global _test_engine
    if _test_engine is None:
        _test_engine = SimpleTestEngine()
    return _test_engine