...
const ChatMessages: React.FC<ChatMessagesProps> = ({
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-gradient-to-b from-gray-50 to-white relative modern-minimal-scrollbar"
      ...
      {!isModelLoaded && (
        <div className="absolute inset-0 bg-white/95 backdrop-blur-xl z-10 flex items-center justify-center">
          <div className="text-center p-12 bg-white rounded-2xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.15)] border border-gray-200">
            <div className="w-16 h-16 mx-auto mb-6 bg-gray-100 rounded-full flex items-center justify-center">
              <div className="w-3 h-3 bg-gray-600 rounded-full"></div>
            </div>
            <p className="text-blue-700 text-lg font-medium">System Offline</p>
            <p className="text-gray-700 text-sm mt-3">Waiting for model initialization</p>
      ...
      {messages.length === 0 && (
        <div className="flex items-center justify-center h-full">
          <div className="text-center">
            ...
            <p className="text-blue-700 text-xl font-medium">Start a Conversation</p>
            <p className="text-gray-700 text-sm mt-2">Select a personality to begin</p>
          </div>
        </div>
      )}
      ...
      <div className="space-y-6 max-w-4xl mx-auto">
              ...
              <div className={`max-w-3xl ${
                ...
                <div className={`px-6 py-4 ${
                  message.role === 'user'
                    ...
                    : message.role === 'system'
                    ? 'bg-gradient-to-r from-amber-50 to-orange-50 text-blue-600 rounded-xl border border-amber-200'
                    : 'bg-white text-blue-700 rounded-2xl rounded-bl-sm shadow-md border border-gray-200'
                }`}>
                ...
                {message.role === 'assistant' && (
                  <div className="mt-2 px-2 text-[11px] text-gray-600 flex items-center gap-2 justify-start">
        ...
        {isLoading && (
          <div className="flex justify-start">
            <div className="px-6 py-4 bg-white rounded-2xl rounded-bl-sm shadow-md border border-gray-200">
              <div className="flex items-center gap-1">
                <span className="w-1.5 h-1.5 bg-gray-600 rounded-full animate-[pulse_1.5s__infinite]"></span>
                <span className="w-1.5 h-1.5 bg-gray-600 rounded-full animate-[pulse_1.5s__infinite_0.2s]"></span>
                <span className="w-1.5 h-1.5 bg-gray-600 rounded-full animate-[pulse_1.5s__infinite_0.4s]"></span>
              </div>
            </div>
          </div>
        )}
