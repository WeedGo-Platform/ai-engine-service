...
const TopMenuBar: React.FC<TopMenuBarProps> = ({
  ...
  return (
    <div className="w-full bg-gradient-to-r from-green-900 via-green-800 to-green-900 px-3 sm:px-6 py-3 sm:py-4 flex items-center justify-between shadow-lg relative overflow-hidden z-40 h-[60px] sm:h-[72px]">
      {/* Left section with logo */}
      <div className="flex items-center gap-4">
        <img src={potPalaceLogo} alt="WeedGo" className="h-6 sm:h-8 w-auto" />
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {isAuthenticated ? (
          <button 
            onClick={logout} 
            className="p-2 sm:p-2.5 rounded-full hover:bg-green-700/50 transition-all group" 
            title="Logout"
          >
            <svg className="w-4 sm:w-5 h-4 sm:h-5 text-green-200 group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        {!isAuthenticated && (
          <button 
            ...
            className="p-2.5 rounded-full hover:bg-green-700/50 transition-all group" 
            title="Sign Up"
          >
        ...
        {isAuthenticated && currentUser && (
          <div className="flex items-center gap-2 px-3 py-1 bg-green-700/50 rounded-full">
            <span className="text-green-200 text-sm font-medium">{currentUser.name || currentUser.email}</span>
          </div>
        )}
      </div>
