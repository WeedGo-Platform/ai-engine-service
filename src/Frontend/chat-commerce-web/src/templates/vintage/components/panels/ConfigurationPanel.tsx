...
const ConfigurationPanel: React.FC<ConfigurationPanelProps> = ({
  ...
  return (
    <div className={`fixed left-0 top-[60px] sm:top-[72px] h-[calc(100vh-60px)] sm:h-[calc(100vh-72px)] w-full sm:w-80 bg-[#FFF8DC] shadow-xl border-r border-[#D2B48C] transform transition-transform duration-300 z-30 ${
      ...
      <div className="p-4 sm:p-6 h-full overflow-y-auto">
        ...
        <div className="mb-6 sm:mb-8">
          <div>
            <h2 className="text-xl sm:text-2xl font-semibold text-[#3E2723]" style={{ fontFamily: "Georgia, serif" }}>
              AI Configuration
            </h2>
            <p className="text-sm text-[#5D4037] mt-1 hidden sm:block">Manage your AI settings</p>
          </div>
        </div>
        ...
        {/* Agent Selector */}
        <div className="mb-8">
          <label className="block text-xs font-semibold text-[#5D4037] uppercase tracking-wider mb-3 px-1" style={{ fontFamily: "Georgia, serif" }}>
            Select Agent
          </label>
          <select
            value={selectedAgent}
            onChange={(e) => setSelectedAgent(e.target.value)}
            className="w-full px-4 py-3 bg-[#FFF8DC] text-[#3E2723] border border-[#D2B48C] rounded-lg focus:outline-none focus:ring-2 focus:ring-[#8B4513] focus:border-transparent text-sm font-medium hover:border-[#8B4513] transition-colors"
        ...
        {currentTemplate && availableTemplates && onTemplateChange && (
          <div className="mb-8">
            <h3 className="text-xs font-semibold text-[#5D4037] mb-4 uppercase tracking-wider px-1" style={{ fontFamily: "Georgia, serif" }}>
              Choose Theme
            </h3>
            <TemplateSwitcher 
        ...
        {/* Voice Settings */}
        <div className="mb-8 p-4 bg-[#F5DEB3] rounded-lg border border-[#D2B48C]">
          <h3 className="text-xs font-semibold text-[#5D4037] mb-4 uppercase tracking-wider" style={{ fontFamily: "Georgia, serif" }}>
            Voice Settings
          </h3>
          {isLoadingVoices ? (
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs text-[#5D4037] mb-2 font-medium" style={{ fontFamily: "Georgia, serif" }}>
                  Voice Selection
                </label>
                <select
                  value={selectedVoice}
                  onChange={(e) => onVoiceChange(e.target.value)}
                  className="w-full px-3 py-2 bg-[#FFF8DC] text-[#3E2723] border border-[#D2B48C] rounded-md focus:outline-none focus:ring-2 focus:ring-[#8B4513] focus:border-transparent text-sm"
                  style={{ fontFamily: "Georgia, serif" }}
                >
              ...
              <div className="flex items-center justify-between">
                <span className="text-xs text-[#5D4037] font-medium" style={{ fontFamily: "Georgia, serif" }}>Text-to-Speech</span>
                <button
          ...
          <div className="mb-8 p-4 bg-[#F5DEB3] rounded-lg border border-[#D2B48C]">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                ...
                <span className="text-[#3E2723] font-medium">{configDetails.name || 'Custom Config'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#5D4037]">Version</span>
                <span className="text-[#3E2723]">{configDetails.version || '1.0.0'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#5D4037]">Temperature</span>
                <span className="text-[#3E2723]">{configDetails.temperature || 'Default'}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#5D4037]">Max Tokens</span>
                <span className="text-[#3E2723]">{configDetails.max_tokens || 'Default'}</span>
              </div>
            </div>
          </div>
        )}
        ...
        {((activeTools && activeTools.length > 0) || isLoadingTools) && (
          <div className="p-4 bg-[#F5DEB3] rounded-lg border border-[#D2B48C]">
            <h3 className="text-xs font-semibold text-[#5D4037] mb-4 uppercase tracking-wider px-1" style={{ fontFamily: "Georgia, serif" }}>
              Active Tools
            </h3>
            {isLoadingTools ? (
              ...
              <div className="space-y-2">
                {activeTools.map(tool => (
                  <div key={tool.name} className="flex items-center gap-3 text-sm">
                    ...
                    <span className="text-[#3E2723]" style={{ fontFamily: "Georgia, serif" }}>{tool.name}</span>
                  </div>
                ))}
              </div>
            )}
