...
export const API_CONFIG = {
  ...
  // Build WebSocket URL from API URL with v2 chat path
  WS_URL: (process.env.EXPO_PUBLIC_API_URL || `http://${LOCAL_IP}:5024`).replace('http://', 'ws://').replace('https://', 'wss://') + '/api/v2/ai-conversation/ws',
