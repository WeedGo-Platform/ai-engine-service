...
class VoiceAuthService:
    ...
    async def extract_voice_features(self, audio_data: bytes) -> np.ndarray:
        ...
        try:
            ...
            # Convert hash to integer values to avoid float32 overflow
            hash_ints = []
            for i in range(0, len(audio_hash), 4):
                ...
                if len(chunk) == 4:
                    ...
                    normalized_val = (int_val / (2**32 - 1)) * 2 - 1  # Scale to [-1, 1]
                    hash_ints.append(normalized_val)
            
            # Repeat pattern to reach desired feature dimension
            while len(hash_ints) < self.feature_dim:
                hash_ints.extend(hash_ints[:min(len(hash_ints), self.feature_dim - len(hash_ints))])
            
            # Convert to numpy array and trim to exact dimension
            ...
            if norm > 1e-8:  # More robust threshold
                ...
                # If norm is still too small, use deterministic initialization
                np.random.seed(int(audio_hash[:4].hex(), 16))  # Deterministic seed
    ...
    async def log_auth_attempt(self, user_id: Optional[str], success: bool, 
        ...
        try:
            ...
            await self.db.execute(
                ...
                user_id,
                success,
                float(score),  # Convert numpy float32 to Python float
                json.dumps(age_info)
            )
        except Exception as e:
