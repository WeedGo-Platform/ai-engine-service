...
class OTPService {
  ...
  async sendOTP(request: OTPSendRequest): Promise<OTPSendResponse> {
    try {
      const response = await api.post('/api/auth/otp/send', request);
  ...
  async verifyOTP(request: OTPVerifyRequest): Promise<OTPVerifyResponse> {
    try {
      const response = await api.post('/api/auth/otp/verify', request);
  ...
  async resendOTP(identifier: string): Promise<OTPSendResponse> {
    try {
      const response = await api.post('/api/auth/otp/resend', { identifier });
      return response.data;
    } catch (error: any) {
